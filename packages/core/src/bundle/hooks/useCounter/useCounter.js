...
export const useCounter = ((...params) => {
    const initialValue = typeof params[0] === 'number'
        ? params[0]
        : params[0]?.initialValue;
    const { max = Number.POSITIVE_INFINITY, min = Number.NEGATIVE_INFINITY } = typeof params[0] === 'number'
        ? (params[1] ?? {})
        : (params[0] ?? {});
    ...
    const set = (value) => {
        setValue((prevValue) => {
            const updatedCount = Math.max(min, Math.min(max, typeof value === 'number' ? value : value(prevValue)));
            return updatedCount;
        });
    };
