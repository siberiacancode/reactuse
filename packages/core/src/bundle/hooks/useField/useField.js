    ...
    const register = (registerParams) => ({
        ref: (node) => {
            if (!inputRef.current && node) {
                ...
                if (registerParams && params?.validateOnMount)
        ...
        onChange: async () => {
            ...
            if (inputRef.current.value !== initialValue)
                setDirty(true);
            if (dirty && inputRef.current.value === initialValue)
                setDirty(false);
            if (registerParams && params?.validateOnChange)
                await validate(registerParams);
            if (registerParams && params?.validateOnBlur)
                setError(undefined);
        },
        onBlur: async () => {
            if (registerParams && params?.validateOnBlur)
