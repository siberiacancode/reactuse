...
export const useMutation = (callback, options) => {
    ...
    const request = (body, requestOptions) => {
        ...
        return callback(body)
            ...
            .catch((error) => {
            const retry = typeof requestOptions?.retry === 'function'
                ? requestOptions?.retry(attempt, error)
                : requestOptions?.retry;
            const retryDelay = typeof requestOptions?.retryDelay === 'function'
                ? requestOptions?.retryDelay(attempt, error)
                : requestOptions?.retryDelay;
            if (typeof retry === 'boolean' && retry) {
                if (retryDelay) {
                    setTimeout(() => request(body, { ...requestOptions, attempt: attempt + 1 }), retryDelay);
                    return;
                }
