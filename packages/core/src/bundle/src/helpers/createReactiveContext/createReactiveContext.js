import { createContext, createElement, startTransition, useContext, useMemo, useRef, useState } from 'react';
...
const createReactiveContextSelector = (Context, selector, options = {}) => {
    ...
    const dispatch = useEvent((newValue) => {
        setValue((prevValue) => {
            if (Object.is(prevValue.value, newValue))
                return prevValue;
            const newSelected = selector(newValue);
            if (Object.is(prevValue.selected, newSelected))
                return prevValue;
