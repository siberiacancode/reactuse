...
export const createStore = (createState) => {
    ...
    const setState = (action) => {
        ...
        if (!Object.is(nextState, state)) {
            ...
            state = (typeof nextState !== 'object' || nextState === null || Array.isArray(nextState)
                ? nextState
    ...
    function useStore(selector) {
        return useSyncExternalStore(subscribe, () => (selector ? selector(getState()) : getState()), () => (selector ? selector(getInitialState()) : getInitialState()));
    }
    return {
        set: setState,
        get: getState,
