...
const resolveAutoIncrement = (progress, trickleRate) => {
    if (progress < 0.25)
        return 0.12 + trickleRate * Math.random();
    if (progress < 0.5)
        return 0.08 + trickleRate * Math.random();
    if (progress < 0.8)
        return 0.03 + trickleRate * Math.random();
    if (progress < 0.95)
        return 0.01 + (trickleRate / 2) * Math.random();
    if (progress < 0.99)
...
export const useProgress = (initialValue = 0, options = {}) => {
    ...
    const inc = (amount = resolveAutoIncrement(value, rate)) => setValue((currentValue) => clamp(currentValue + amount, initialValue, maximum));
