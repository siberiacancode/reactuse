...
export const useSticky = ((...params) => {
    ...
    useEffect(() => {
        ...
        const elementOffsetTop = element.getBoundingClientRect().top + root.scrollTop - root.getBoundingClientRect().top;
        const elementOffsetLeft = element.getBoundingClientRect().left + root.scrollLeft - root.getBoundingClientRect().left;
        const onSticky = () => {
            if (axis === 'vertical') {
