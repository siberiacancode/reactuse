...
it('Should use notifications on server side', () => {
  const { result } = renderHookServer(() => useNotifications());
...
it('Should use notifications for unsupported browser', () => {
  Object.defineProperty(window, 'Notification', {
    value: undefined,
    writable: true,
    configurable: true
...
it('Should merge default options with show overrides', async () => {
  ...
  const { result } = renderHook(() => useNotifications({ title: 'Default', body: 'Default body' }));
...
it('Should call onClick callback when notification is clicked', async () => {
  ...
  const { result } = renderHook(() => useNotifications({ onClick }));
...
it('Should call onShow callback when notification is shown', async () => {
  ...
  const { result } = renderHook(() => useNotifications({ onShow }));
...
it('Should call onError callback when notification errors', async () => {
  ...
  const { result } = renderHook(() => useNotifications({ onError }));
...
it('Should call onClose callback and clear notification when closed', async () => {
  ...
  const { result } = renderHook(() => useNotifications({ onClose }));
