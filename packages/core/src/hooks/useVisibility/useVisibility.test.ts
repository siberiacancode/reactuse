...
import type { UseVisibilityReturn } from './useVisibility';
...
targets.forEach((target) => {
  describe(`${target}`, () => {
    it('Should use intersection observer', () => {
      const { result } = renderHook(() => {
        if (target)
          return useVisibility(target) as unknown as {
            ref: StateRef<HTMLDivElement>;
          } & UseVisibilityReturn;
        return useVisibility<HTMLDivElement>();
      });
    ...
    it('Should update inView state when element enters viewport', () => {
      const { result } = renderHook(() => {
        if (target)
          return useVisibility(target) as unknown as {
            ref: StateRef<HTMLDivElement>;
    ...
    it('Should call onChange callback when intersection', () => {
      ...
      const { result } = renderHook(() => {
        if (target)
          return useVisibility(target, {
            onChange
          }) as unknown as {
            ref: StateRef<HTMLDivElement>;
          } & UseVisibilityReturn;
        return useVisibility<HTMLDivElement>({ onChange });
      });
    ...
    it('Should call callback on intersection', () => {
      ...
      const { result } = renderHook(() => {
        if (target)
          return useVisibility(target, callback) as unknown as {
            ref: StateRef<HTMLDivElement>;
          } & UseVisibilityReturn;
        return useVisibility<HTMLDivElement>(callback);
      });
    ...
    it('Should handle options properly', () => {
      ...
      const { result } = renderHook(() => {
        if (target)
          return useVisibility(target, options) as unknown as {
            ref: StateRef<HTMLDivElement>;
          } & UseVisibilityReturn;
        return useVisibility<HTMLDivElement>(options);
      });
  ...
  it('Should handle enabled option', () => {
    const { result } = renderHook(() => useVisibility<HTMLDivElement>({ enabled: false }));
