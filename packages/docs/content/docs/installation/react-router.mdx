...
This will create a configuration file [`reactuse.json`](../reactuse-json.md) in your project. Ensure your `tsconfig.json` has path aliases (React Router often uses `~/*`). If you use `@docs/*`, add:
...
If your `reactuse.json` uses `@docs/shared/hooks`, use that alias instead of `~/shared/hooks`.
