...
export async function generateStaticParams() {
  return source
    .generateParams()
    .filter((entry) => entry.slug?.length === 2)
    .map((entry) => ({
      type: entry.slug[0],
      name: entry.slug[1]
    }));
...
export async function generateMetadata(props: { params: Promise<{ name: string; type: string }> }) {
  const { type, name } = await props.params;
  const page = source.getPage([type, name]);
  if (!page) {
    notFound();
  }
...
const MdxPage = async ({ name, type }: { name: string; type: string }) => {
  const page = source.getPage([type, name]);
  if (!page) {
    notFound();
  }
...
export default async function Page(props: { params: Promise<{ name: string; type: string }> }) {
  const { type, name } = await props.params;
  return <MdxPage name={name} type={type} />;
}
