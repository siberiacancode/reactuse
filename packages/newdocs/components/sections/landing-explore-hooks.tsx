...
export const LandingExploreHooks = ({ hooks }: LandingExploreHooksProps) => {
  const rows = Array.from({ length: ROW_COUNT }, (_, rowIndex) =>
    Array.from(
      { length: PER_ROW },
      (_, index) => hooks[(rowIndex * PER_ROW + index) % hooks.length]
    )
  );
        ...
        <div className='relative mt-12 overflow-hidden'>
          <div className='space-y-4 py-4'>
            {rows.map((row, index) => (
              <div key={`row-${index}`}>
                <div
                  className={index % 2 === 0 ? 'landing-hooks-row-left' : 'landing-hooks-row-right'}
                >
                  {row.map((hook) => (
                    <Link
                      key={hook}
          ...
          <div className='from-background via-background/30 pointer-events-none absolute inset-y-0 left-0 w-12 bg-gradient-to-r to-transparent' />
          <div className='from-background via-background/30 pointer-events-none absolute inset-y-0 right-0 w-12 bg-gradient-to-l to-transparent' />
        </div>
