---
title: "useMemory"
description: "Hook that gives you current memory usage"
---

import { Code } from '@docs/components/code';
import { CodeBlockCommand } from '@docs/components/code-block-command';
import { DocContributors, DocHeader, DocTableApi, DocUsageExamples } from '@docs/components/hook-doc-page';
import { HookDemo } from '@docs/components/hook-demo';
import { ViewCode } from '@docs/components/view-code';
import hookDoc from './useMemory.props.json';
import { Separator } from '@docs/ui/separator';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@docs/ui/tabs';

<DocHeader {...hookDoc} />

<Separator className="my-2" />
## Installation

<Tabs defaultValue="library">
  <TabsList>
    <TabsTrigger value="library">Library</TabsTrigger>
    <TabsTrigger value="cli">CLI</TabsTrigger>
    <TabsTrigger value="manual">Manual</TabsTrigger>
  </TabsList>

  <TabsContent className="space-y-3" value="library">
    <Code code={`import { useMemory } from '@siberiacancode/reactuse';`} />
  </TabsContent>

  <TabsContent className="space-y-3" value="cli">
    <CodeBlockCommand __bun__="bunx --bun useverse@latest add useMemory" __npm__="npx useverse@latest add useMemory" __pnpm__="pnpm dlx useverse@latest add useMemory" __yarn__="yarn useverse@latest add useMemory" />
  </TabsContent>

  <TabsContent className="space-y-3" value="manual">
    <p className="text-muted-foreground mb-3 text-sm">
      Copy and paste the following code into your project. Update the import paths to match your project setup.
    </p>
    <ViewCode lineCount={(hookDoc.sourceCode ?? '').split('\n').length}>
      <Code code={hookDoc.sourceCode ?? ''} />
    </ViewCode>
  </TabsContent>
</Tabs>

<Separator className="my-2" />
## Usage

<DocUsageExamples {...hookDoc} />

<Separator className="my-2" />
## Demo

<HookDemo {...hookDoc} />

<Separator className="my-2" />
## API

<DocTableApi {...hookDoc} />

<Separator className="my-2" />
## Type declaration

  <ViewCode lineCount={(hookDoc.typeDeclarations ?? '').split('\n').length}>
    <Code code={hookDoc.typeDeclarations ?? ''} />
  </ViewCode>

<Separator className="my-2" />
## Source

[Source](https://github.com/siberiacancode/reactuse/blob/main/packages/core/src/hooks/useMemory/useMemory.ts)

[Demo](https://github.com/siberiacancode/reactuse/blob/main/packages/core/src/hooks/useMemory/useMemory.demo.tsx)

<Separator className="my-2" />
## Contributors

<DocContributors {...hookDoc} />
