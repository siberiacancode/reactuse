...
const Demo = () => {
  ...
  const onCapture = (snapshot?: string) => {
    if (!snapshot) return;
  ...
  return (
    <section className='flex justify-center'>
      <div className='relative flex h-[520px] w-80 flex-col overflow-hidden rounded-4xl border bg-black'>
        <div className='bg-background absolute top-3 left-1/2 z-20 h-6 w-24 -translate-x-1/2 rounded-full' />
              ...
              {ready && (
                <div className='absolute inset-x-0 bottom-0 z-10 flex flex-col items-center gap-3 px-6 pb-10'>
                  ...
                  <button
                    ...
                    type='button'
                    onClick={() => onCapture(getSnapshot({ quality: 0.9 }))}
                  />
                </div>
              )}
            </>
