---
title: Tracking the mouse in React without drowning in re-renders
description: reactuse ships browser-event hooks like useMouse with a callback-first API and an opt-in watch() method — so you get direct DOM control by default and reactive state only when you actually need it.
date: 2026-08-08
---

reactuse ships 100+ hooks, and a whole family of them exists for one reason: the browser is a firehose of events. Scroll, resize, pointer movement, key presses — these fire dozens or hundreds of times per second. Hooks like [`useScroll`](https://reactuse.org/functions/hooks/useScroll), [`useWindowScroll`](https://reactuse.org/functions/hooks/useWindowScroll), [`useResizeObserver`](https://reactuse.org/functions/hooks/useResizeObserver), and [`useMouse`](https://reactuse.org/functions/hooks/useMouse) wrap those noisy event streams so you don't have to wire up listeners, compute deltas, and remember to clean everything up on unmount.
...
const Demo = () => {
  ...
  return (
    <div ref={mouse.ref}>
      {/* elementX / elementY are relative to the element,
          x / y are relative to the page */}
      position: {mouse.snapshot.elementX}, {mouse.snapshot.elementY}
    </div>
  );
};
...
You can also pass a target directly (like `window`) instead of using the returned ref — the same hook handles both. That flexibility with refs is nice, but it isn't the interesting part.
...
Most libraries "solve" this by bolting throttling onto the hook: `useMouse({ throttleMs: 16 })`. We think that's the wrong call, and we'll come back to why at the end.

reactuse takes a different route. **By default, `useMouse` does not re-render your component at all.**
...
It's tempting to add `useMouse({ throttleMs: 16 })` and call it a day. We deliberately don't, and the reason is about API responsibility.
...
Want the full API? Head to the [useMouse docs](https://reactuse.org/functions/hooks/useMouse), pair it with [`useThrottleCallback`](https://reactuse.org/functions/hooks/useThrottleCallback) when you need it, and browse the rest of the [skills and hooks](/docs/skills) while you're there. 🧊
