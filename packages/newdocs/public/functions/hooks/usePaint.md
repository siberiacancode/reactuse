...
`UsePaintReturn` - An object containing the current brush state and functions to interact with the paint
...
`UsePaintReturn & { ref: StateRef<HTMLCanvasElement> }` - An object containing the current brush state and functions to interact with the paint